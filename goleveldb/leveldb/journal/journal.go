...
//日志数据也是按照chunk堆积的
func (r *Reader) nextChunk(first bool) error { //headersize + 4 BYTE的checksum + 2BYTE的数据长度+1BYTE的CHUNK TYPE+DATA
	for {
		if r.j+headerSize <= r.n {
			checksum := binary.LittleEndian.Uint32(r.buf[r.j+0 : r.j+4]) //校验和
			length := binary.LittleEndian.Uint16(r.buf[r.j+4 : r.j+6]) //本chunk数据长度
...
func (w *Writer) Next() (io.Writer, error) {
	...
	if w.pending {//如果上一次数据都写完毕了，还剩余最后一个未满的block未刷入到磁盘，此时触发写入（数据恢复时用到）
		w.fillHeader(true)
	}
	w.i = w.j
	w.j = w.j + headerSize
