...
func (h *dbHarness) maxNextLevelOverlappingBytes(want int64) {
	...
	v := db.s.version()
	if len(v.versionLevels) > 2 {
		for i, tt := range v.versionLevels[1 : len(v.versionLevels)-1] {
			level := i + 1
			next := v.versionLevels[level+1]
			for _, t := range tt {
...
func (h *dbHarness) totalTables() (n int) {
	v := h.db.s.version()
	for _, tables := range v.versionLevels {
		n += len(tables)
	}
	v.release()
	return
...
func Test_FieldsAligned(t *testing.T) {
	p1 := new(DB)
	testAligned(t, "DB.seq", unsafe.Offsetof(p1.diskSeq))
	p2 := new(session)
...
func TestDB_Write(t *testing.T) {
	trun(t, func(h *dbHarness) {
		for i := 1000000; i > 0; i-- {
			j := rand.Int()
			h.put(numKey(j), strings.Repeat(fmt.Sprintf("v%09d", i), 1000/10))
			h.delete(numKey(j))
			h.put(numKey(j), strings.Repeat(fmt.Sprintf("v%09d", i), 1000/10))

			//h.put(numKey(999), strings.Repeat(fmt.Sprintf("v%09d00000", 999), 1000/10))
		}
	})
}
...
func TestDB_UkeyShouldntHopAcrossTable(t *testing.T) {
	...
	h.waitCompaction()
	for level, tables := range h.db.s.stVersion.versionLevels {
...
func TestDB_TableCompactionBuilder(t *testing.T) {
	...
	// Build grandparent.
	v := s.version()
	c := newCompaction(s, v, 1, append(tFiles{}, v.versionLevels[1]...))
	rec := &sessionRecord{}
	...
	// Build level-1.
	v = s.version()
	c = newCompaction(s, v, 0, append(tFiles{}, v.versionLevels[0]...))
	rec = &sessionRecord{}
	b = &tableCompactionBuilder{
		s:         s,
		c:         c,
	...
	for _, t := range v.versionLevels[2] {
	...
	for level, want := range []bool{false, true, false, true} {
		got := len(v.versionLevels[level]) > 0
		if want != got {
			t.Fatalf("invalid level-%d tables len: want %v, got %v", level, want, got)
		}
	}
	for i, f := range v.versionLevels[1][:len(v.versionLevels[1])-1] {
		nf := v.versionLevels[1][i+1]
	...
	v = s.version()
	if len(v.versionLevels[1]) != len(v.versionLevels[2]) {
		t.Fatalf("invalid tables length, want %d, got %d", len(v.versionLevels[1]), len(v.versionLevels[2]))
	}
	for i, f0 := range v.versionLevels[1] {
		f1 := v.versionLevels[2][i]
