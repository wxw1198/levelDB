	...
	if r.cache != nil {
		...
		if fillCache {
			ch = r.cache.Get(bh.offset, func() (size int, value cache.Value) {
				var b *block
				//数据缓存在cache中。
				//数据读取到block里面，block存储于handle指针中，即变量ch里面。
...
func (r *Reader) find(key []byte, filtered bool, ro *opt.ReadOptions, noValue bool) (rkey, value []byte, err error) {
	...
	if filtered && r.filter != nil {
		...
		if ferr == nil { //filter是否包含key,如果不包含则此tableFile中确定没有key
	...
	if !data.Seek(key) { //为什么会出现本dataBlock中的数据没有key, 还需要找下一个dataBlock的情况？????
		data.Release()
