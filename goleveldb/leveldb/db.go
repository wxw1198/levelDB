...
type DB struct {
	...
	mem, frozenMem  *memDB //当前的内存数据库和上一个内存数据库
...
func (db *DB) recoverJournal() error {
	...
	var (
		ofd storage.FileDesc // Obsolete file.废弃的文件
		rec = &sessionRecord{}
	)
	...
	if len(fds) > 0 {
		...
		for _, fd := range fds {
			...
			//有多个日志文件的时候，先把前面的文件处理完毕（文件内容刷入地盘/文件被删除），再处理后面的日志文件
			// 这块代码写的有些乱，原因是每个日志文件被删除前，要确保它的内容被写入到数据库；而写入数据库的操作可能失败，因此这里确保一个日志文件的内容被写入后，才删除对应的日志文件。
			...
			for {
				...
				batchSeq, batchLen, err = decodeBatchToMem(buf.Bytes(), db.diskSeq, mdb)
				if err != nil {
				...
				db.diskSeq = batchSeq + uint64(batchLen)

				// Flush it if large enough.
				if mdb.Size() >= writeBuffer { //恢复到的数据足够多，刷到leveldb level 0里面
					if _, err := db.s.flushMemdb(rec, mdb, 0); err != nil {
						fr.Close()
						return err
					}
			...
			fr.Close()
			ofd = fd //文件被恢复后，文件信息放入到废弃文件集合中
		}

		// Flush the last memdb.
		if mdb.Len() > 0 { //日志文件恢复完毕后，只要有数据，就立即刷入到level:0的DB中
			if _, err := db.s.flushMemdb(rec, mdb, 0); err != nil {
				return err
			}
		}
