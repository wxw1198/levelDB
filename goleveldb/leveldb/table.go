...
// expanded.
// The dst content will be overwritten.返回重叠的文件
func (tf tFiles) getOverlaps(dst tFiles, icmp *iComparer, umin, umax []byte, overlapped bool) tFiles {
	...
	//对于非零级别，根本没有重叠的ukey。这些级别中的文件是严格排序的，因此使用二进制搜索。
	if !overlapped {
		var begin, end int
	...
	for i := 0; i < len(tf); {
		t := tf[i]
		if t.overlaps(icmp, umin, umax) {//表中的Key,与给的key范围存在重叠的
...
type tOps struct {
	...
	noSync       bool
	evictRemoved bool
	cache        *cache.Cache //表索引缓存
	bcache       *cache.Cache //dataBlock缓存
