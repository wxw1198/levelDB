...
//填充key value 到 iter里面
func (i *dbIter) fill(checkStart, checkLimit bool) bool {
	if i.node != 0 {
		n := i.p.nodeData[i.node]
		m := n + i.p.nodeData[i.node+nKey]
		i.key = i.p.kvData[n:m]
		if i.slice != nil { //校验key是否正确
			switch {
...
func (i *dbIter) First() bool {
	...
	if i.slice != nil && i.slice.Start != nil {//从设置的key找数据位置
...
type DB struct {
	...
	// [3..height] : Next nodes
	nodeData  []int //跳表的索引
	prevNode  [tMaxHeight]int //节点的在每一层的前驱节点
	maxHeight int
	n         int
	kvSize    int
}
...
func (p *DB) findGE(key []byte, prev bool) (int, bool) {
	...
	h := p.maxHeight - 1
	for {
		next := p.nodeData[node+nNext+h] //前12个索引留给每层的第一个node,先从最高层找
		cmp := 1
		if next != 0 {
			o := p.nodeData[next]
		...
		if cmp < 0 {
			...
			node = next //key大一些，在本层继续找下一个节点
		} else {
			if prev {
				p.prevNode[h] = node //记录查询的先前的节点
			} else if cmp == 0 {
				return next, true
			}
			//根据跳表的特征，每个节点在n+1层存在，那么在n层就存在，因此即使comp=0,可以继续向下一层找>=key的具体位置
			if h == 0 {
				return next, cmp == 0
			}
			h-- //到下一层继续查找
...
func (p *DB) Put(key []byte, value []byte) error {
	...
	//跳表实现
	if node, exact := p.findGE(key, true); exact {//如果key已经存在；覆盖老的kv值对
		kvOffset := len(p.kvData)
		p.kvData = append(p.kvData, key...)
		p.kvData = append(p.kvData, value...)
		p.nodeData[node] = kvOffset //记录更新后的位置
	...
	h := p.randHeight() // 随机一个高度，决定key的最高层
	if h > p.maxHeight {
		for i := p.maxHeight; i < h; i++ {
			p.prevNode[i] = 0 //未设置的先前层的节点清零
		}
		p.maxHeight = h //最大的高度从1逐渐提高到11
	}
	...
	for i, n := range p.prevNode[:h] {
		m := n + nNext + i
		p.nodeData = append(p.nodeData, p.nodeData[m]) //其他层的高度，从低到高排序
		p.nodeData[m] = node
	}
...
func (p *DB) Delete(key []byte) error {
	...
	for i, n := range p.prevNode[:h] {
		m := n + nNext + i
		p.nodeData[m] = p.nodeData[p.nodeData[m]+nNext+i] //类似链表操作，前驱指向后继
	}
...
func New(cmp comparer.BasicComparer, capacity int) *DB {
	p := &DB{
		...
		nodeData:  make([]int, 4+tMaxHeight), //tMaxHeight这么大的空间，用来存放每一层的第一个节点位置
	}
	p.nodeData[nHeight] = tMaxHeight
	return p
}
