...
func (db *DB) checkAndCleanFiles() error {
	...
	for _, fd := range fds {
		...
		case storage.TypeManifest:
			keep = fd.Num >= db.s.manifestFd.Num //小于当前正在使用的manifest，就删除掉
		case storage.TypeJournal:
			if !db.frozenJournalFd.Zero() {
				//如果有被冻结的日志文件存在，那么只要文件序列号大于被冻结的，就不能丢弃。frozen的日志文件对应的数据要写到磁盘后，对应的日志才会被删除
				keep = fd.Num >= db.frozenJournalFd.Num
			} else {
				// 老的日志文件被删除，新的日志文件保存下来
	...
	if nt != len(tmap) { //版本信息中和读取的实际文件信息必须相等，否则恢复过程是错误的
		var mfds []storage.FileDesc
		for num, present := range tmap {//文件丢失，记录丢失的文件序列号到日志中
			if !present {
