...
func (db *DB) memCompaction() {
	...
	//内存数据库压缩的时候，暂停Table压缩，然后再启动table 压缩
	...
	// Commit.
	stats.startTimer()
	//将fulshmemdb的结果进行提交，并记录日志，提交的过程主要是为了将新生成的表信息写入到MANIFEST文件中，同时生成新的version。
	...
	db.compStats.addStat(flushLevel, stats) //统计信息
...
func (b *tableCompactionBuilder) needFlush() bool {
	//fmt.Println("needFlush:", b.tw.tw.BytesLen(), ">=", b.tableSize)
...
func (b *tableCompactionBuilder) run(cnt *compactionTransactCounter) error {
	...
	//所有的table文件，通过iter逐个kv读取
	iter := b.c.newIterator()
	defer iter.Release()
	// Next每次取的就是当前所有打开文件的最小的key
	//key 一样的情况下，序列号从大到小排列
	for i := 0; iter.Next(); i++ {
		...
		if kerr == nil {
			...
			if !hasLastUkey || b.s.icmp.uCompare(lastUkey, ukey) != 0 {
				...
				//数据量较多(2mb一个表文件)或者与高级的表文件重叠的数据达到阈值，先刷一部分进入到新的文件中
			...
			switch {
			case lastSeq <= b.minSeq: //未被压缩的表，对应的最大序列号; lastSeq不能等于keyMaxSeq时，要么之前的数据刷入磁盘，要么前一个key，与当前的key相等。
			...
			// Don't drop corrupted keys.尽管不能解析出ikey，仍然把value 写入到新表中
			hasLastUkey = false
			lastUkey = lastUkey[:0]
			lastSeq = keyMaxSeq
			b.kerrCnt++
	...
	// Finish last table.所有的数据写入到磁盘，并且关闭文件句柄。
...
func (db *DB) tableCompaction(c *compaction, noTrivial bool) {
	...
	// 因为source+1层的文件个数是0，因此能保证同一层的key不重叠
	...
	// Commit.
	stats[1].startTimer()
	// commit compaction,完成新旧版本的替换级日志记录：
	// 1. 生成新的version，并计算cSocore、cLevel
...
func (db *DB) tableRangeCompaction(level int, umin, umax []byte) error {
	...
	if level >= 0 { //指定压缩级别level
		if c := db.s.getCompactionRange(level, umin, umax, true); c != nil {
			db.tableCompaction(c, true) //启动压缩
		}
	} else {
		...
		for { //压缩所有的level
			...
			v := db.s.version()
			m := 1
			for i := m; i < len(v.versionLevels); i++ {
				tables := v.versionLevels[i]
				if tables.overlaps(db.s.icmp, umin, umax, false) {
					m = i //获取压缩波及的级别范围。如果新来的table中的Key跨度较大，影响的文件会很多效率会很低？？？？
				}
			}
			v.release()

			for level := 0; level < m; level++ { //获取压缩的时候，umin/umax都可能被修改，因此需要一个循环来实现压缩的完整性
	...
	for {
		if db.tableNeedCompaction() {
			...
			if len(waitQ) > 0 && db.resumeWrite() {
				for i := range waitQ { //积累一定数量的表后，才进行压缩操作
					waitQ[i].ack(nil)
					waitQ[i] = nil
				}
				waitQ = waitQ[:0]
