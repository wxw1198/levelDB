...
func (db *DB) getSeq() uint64 {
	return atomic.LoadUint64(&db.diskSeq)
}

// Atomically adds delta to seq.
func (db *DB) addSeq(delta uint64) {
	atomic.AddUint64(&db.diskSeq, delta)
	//fmt.Println("db seq:", db.seq)
}

func (db *DB) setSeq(seq uint64) {
	atomic.StoreUint64(&db.diskSeq, seq)
}
...
func (db *DB) newMem(n int) (mem *memDB, err error) {
	...
	//老的内存数据库，没有写入磁盘，不能生成新的内存数据库
	if db.frozenMem != nil {
		return nil, errHasFrozenMem
	}

	//每个memDB对应一个journal 文件
	if db.journal == nil {
