...
type session struct { //session table
	// Need 64-bit alignment.
	stNextFileNum    int64 // current unused file number
	stJournalNum     int64 // current journal file number; need external synchronization已经确认过的日志
...
// 提交版本记录的时候，同时计算各个级别的分数，为下一轮压缩做准备
